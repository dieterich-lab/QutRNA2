...
[QutRNA2](https://github.com/dieterich-lab/JACUSA2
[JACUSA2 2.1.15](https://github.com/dieterich-lab/JACUSA2)
[parasail v2.6.2](https://github.com/jeffdaily/parasail/archive/refs/tags/v2.6.2.tar.gz)
[gpu-tRNA-mapper](https://github.com/fkallen/gpu-tRNA-mapper)
...
### JACUSA2
Download [JACUSA2 2.1.15](https://github.com/dieterich-lab/JACUSA2) and store the JAR on your local computer. Put the JAR into a directory in your PATH or remember the location <JACUSA2_LOCAL_JAR> for later configuration.
...
When you are in <GPU-TRNA-MAPPER-LOCAL-DIR>, you can start building the gpu-tRNA-mapper - use <PARASAIL-LOCAL-DIR> from the building parasail:
```console
make \
...
Second, define your <DATA_YAML>. This file describes what reference and Sprinzl coordinates (if any) to use. See `examples/data.yaml`. Make sure to add your <SAMPLE_DESC>. Provide "ref_fasta" and define what Sprinzl coordinates to use, as well as the size of the adapters used!
Correct adapter lengths are essential!

### Sprinzl
For eukaryotic nuclear tRNAs, we used the following covariance model [TRNAinf-euk.cm](https://github.com/UCSC-LoweLab/tRAX/blob/master/TRNAinf-euk.cm) and labeling `data/nuclear-euk-masked.txt`.

For human mt-tRNAs, we used the sequence to Sprinzl mapping in [https://www.nature.com/articles/s41467-020-18068-6](https://www.nature.com/articles/s41467-020-18068-6) 
and deposited in the data along with the Sprinzl labels: `data/human_mt_seq_to_sprinzl.tsv` and `data/human_mt_sprinzl_labels.txt`. If you provide your own labels make sure that the first label is a "-"!
...
Use <ANALYSIS_OUTPUT> folder to define where QutRNA2 should write its output to:
```console
snakemake \
        -c 1 \
...
## Results
When the analysis is finished, the <ANALYSIS_OUTPUT> will contain the following subdirectories:
"data", "info", "logs", and "results".

`<ANALYSIS_OUTPUT>/data/` will contain all unprocessed data that was used in the analysis.
...
## plots
The directory `<ANALYSIS_OUTPUT>/results/plots` will contain 

Check `<ANALYSIS_OUTPUT>/results/plots/scores/cond1~/cond2~/{id}/bam~final/heatmap.pdf` for JACUAS2 score profiles after filtering. This plot concludes the analysis.
However, differently customized versions might be added in <ANALYSIS_YAML>.
